...
def parse_mjcf_obsolete(
    ...
    def parse_body(body, parent):
        ...
        for i, joint in enumerate(body.findall("joint")):
            ...
            if (joint_limited):
                if radians:
                    joint_limit_lower=np.rad2deg(joint_range[0])
                    joint_limit_upper=np.rad2deg(joint_range[1])
                else:
                    joint_limit_lower=joint_range[0]
                    joint_limit_upper=joint_range[1]
            else:
                joint_limit_lower=-1.e+6
                joint_limit_upper=1.e+6
            ...
            link = builder.add_link(
                ...
                axis=joint_axis, 
                type=joint_type,
                limit_lower=np.deg2rad(joint_limit_lower),
                limit_upper=np.deg2rad(joint_limit_upper),
                limit_ke=limit_ke,
                limit_kd=limit_kd,
                stiffness=joint_stiffness,
                damping=damping,
        ...
        for geom in body.findall("geom"):
            ...
            if (geom_type == "sphere"):
                ...
                if ("fromto" in geom.attrib):
                    ...
                    geom_rot = df.quat_multiply(df.quat_from_axis_angle(axis_angle[0:3], axis_angle[3]),df.quat_from_axis_angle((0.0, 1.0, 0.0), -math.pi*0.5))
